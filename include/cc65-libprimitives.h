...
const char* CreateSphere(int *nVerts, int *nTris, fix8 **verts, fix8 **norms, int **tris, int **pxls)
{
	...
	for (n = 0; n < (*nTris); ++n) {
		...
		// Average coords	
		WriteFix8(norms, n*3+0, 2*(ReadFix8(verts, vertices[0]*3+0) + ReadFix8(verts, vertices[1]*3+0) + ReadFix8(verts, vertices[2]*3+0))/3);
		WriteFix8(norms, n*3+1, 2*(ReadFix8(verts, vertices[0]*3+1) + ReadFix8(verts, vertices[1]*3+1) + ReadFix8(verts, vertices[2]*3+1))/3);
		WriteFix8(norms, n*3+2, 2*(ReadFix8(verts, vertices[0]*3+2) + ReadFix8(verts, vertices[1]*3+2) + ReadFix8(verts, vertices[2]*3+2))/3);
	}
